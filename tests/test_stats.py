...
def test_gerar_stats_basico(watched_csv, tmp_path):
...
def test_gerar_stats_com_ratings(watched_csv, ratings_csv, tmp_path):
    out = tmp_path / "stats.json"
...
def test_heatmap_tem_datas_corretas(watched_csv, tmp_path):
    out = tmp_path / "stats.json"
    gerar_stats(watched_csv, out)
    data = json.loads(out.read_text(encoding="utf-8"))
...
def test_by_year_correto(watched_csv, tmp_path):
    out = tmp_path / "stats.json"
    gerar_stats(watched_csv, out)
    data = json.loads(out.read_text(encoding="utf-8"))
...
def test_top_day_correto(watched_csv, tmp_path):
    out = tmp_path / "stats.json"
    gerar_stats(watched_csv, out)
    data = json.loads(out.read_text(encoding="utf-8"))
...
def test_ratings_distribuicao(watched_csv, ratings_csv, tmp_path):
    out = tmp_path / "stats.json"
    gerar_stats(watched_csv, out, ratings_csv)
    data = json.loads(out.read_text(encoding="utf-8"))
...
def test_nota_maxima_lista(watched_csv, ratings_csv, tmp_path):
    out = tmp_path / "stats.json"
    gerar_stats(watched_csv, out, ratings_csv)
    data = json.loads(out.read_text(encoding="utf-8"))
